...
# Πηγή: Swagger UI (Companies -> GET /companies), Base URL στο header. (δείτε docs) :contentReference[oaicite:1]{index=1}
GEMI_BASES = [
    "https://opendata-api.businessportal.gr/api/opendata/v1",   # σωστό base
    "https://opendata-api.businessportal.gr/opendata",           # legacy
    "https://opendata-api.businessportal.gr",                    # υποθετικό legacy
]

GEMI_HEADER_NAME = "api_key"  # από τα docs/authorize (header name)
TIMEOUT = 40

def _headers(api_key: str):
    return {GEMI_HEADER_NAME: api_key, "Accept": "application/json"}

def _first_ok_url(session: requests.Session, urls: list[str]) -> str | None:
    last_err = None
    for u in urls:
        try:
...
def _bases():
    # Προστασία από ελληνικό 'ο' στο 'opendata'
    return [b.replace("οpendata", "opendata") for b in GEMI_BASES]

def gemi_params(api_key: str, what: str, *, region_id=None):
    """
    Παίρνει παραμετρικά (regions / regional_units / prefectures / nomoi / perifereiakes_enotites / dimoi / municipalities / statuses / kad)
    από /api/opendata/v1/params/* με αρκετά fallbacks.
    """
    ...
    if what == "regions":
        ...
        if region_id is not None:
            endpoints = [
                ...
                f"params/prefectures/{region_id}",   # μερικές φορές έτσι το λένε
            ]
        else:
    ...
    for base in base_list:
        for ep in endpoints:
            ...
            try:
                ...
                pass
    raise RuntimeError("ΓΕΜΗ: δεν βρέθηκε endpoint για '%s'. Δοκιμάστηκαν: %s" % (what, " | ".join(tried[-6:])))
...
def gemi_companies_search(api_key: str, *,
    ...
    Χρησιμοποιεί πολλαπλά aliases παραμέτρων (snake/camel) για συμβατότητα.
    """
    session = requests.Session()
    session.headers.update(_headers(api_key))
    last_err = None

    # Παραλλαγές παραμέτρων (μερικοί servers δέχονται διαφορετικές ονομασίες)
    variants = [
        {
    ...
    for base in _bases():
        ...
        for params in variants:
            # καθάρισε None params
            q = {k: v for k, v in params.items() if v not in (None, "", [])}
            try:
                r = session.get(url, params=q, timeout=TIMEOUT)
                if r.status_code == 200:
                    return r.json()
                # 400: ίσως λάθος όνομα παραμέτρου – δοκίμασε άλλη παραλλαγή
...
def gemi_companies_all(api_key: str, *,
                       ...
                       status_id=None, kad_list=None,
                       date_from=None, date_to=None,
                       per_page=200, max_pages=100, sleep_sec=0.2):
    items = []
    for p in range(1, max_pages + 1):
        ...
        items.extend(arr)
        # τερματισμός όταν δεν έχει άλλα
...
# Αν επιλεγεί ΓΕΜΗ, φτιάχνουμε UI για φίλτρα και κατέβασμα
gemi_df = None
if biz_source == "ΓΕΜΗ (OpenData API)":
    if not gemi_key:
        ...
        try:
            ...
            # Φόρτωσε λίστες με fallbacks
            try:
                regions = gemi_params(gemi_key, "regions")
            except Exception:
                regions = []
            ...
            if sel_region_id:
                try:
                    regional_units = gemi_params(gemi_key, "regional_units", region_id=sel_region_id)
                except Exception:
                    regional_units = []
            runit_map = {}
            ...
            if sel_runit_id:
                try:
                    municipalities = gemi_params(gemi_key, "dimoi", region_id=sel_runit_id)
                except Exception:
                    municipalities = []
            muni_map = {}
