...
+    """
+    Φορτώνει παραμετρικά από ΓΕΜΗ με fallbacks (έχουν αλλάξει slugs παλαιότερα).
+    """
...
+    """
+    Αναζήτηση εταιρειών: δοκιμάζει εναλλακτικά paths & payloads.
+    """
...
+            except requests.RequestException as e:
+                last_err = str(e)
+        # GET fallback (μερικές εκδόσεις το δέχονται)
+        try:
